...
Wordle is a web-based word game created by Josh Wardle and was recently purchased by the New York Times. The simple word-guessing game has over 300,000 people attempting to guess "today's wordle" each and every day. 
...
### Vowel Strategy
The first strategy considered was the vowel strategy. All English words (with some exceptions) have a vowel in them, and therefore eliminating the maximum number of vowels in a starter word should theoretically remove a maximum number of words from the dataset.
...
The vowels `a` and `e` were evidently more frequent that `i`, `o`, and `u`. In fact, having `e` in the starter word would immediately eliminate over 50% of the words in the dataset.
The best words according to the vowel strategy were `aurei` and `uraei` which both eliminated 95.35% of all possibilities. 
...
To my surprise, neither the vowel or letter frequency strategies yielded the best results. The "best" wordle starter according to the brute force strategy is `toeas`, which eliminates 95.72% of all word possibilities. Using this word as a starter could narrow the dataset from 12,972 words to just 555 words or lower. We can therefore conclude that the best wordle starter word is `toeas`.

### Extension
While `toeas` is certainly the best word to use in order to eliminate the maximum number of possible words, it will never be the final solution to a Wordle. This is because each Wordle's solution needs to be a word commonly used in the English language. Therefore, only 2,315 words out of the 12,900 can actually be the solution to the Wordle. For people who want to have a chance of guessing a Wordle on their first attempt, we would have to find a word that is both optimal and is present in the `answers` dataset. Seeing as the brute force strategy yielded the highest scoring words, I decided to use that again with the `answers` dataset. 
